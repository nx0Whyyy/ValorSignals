...
**Version 2.2.0** — Dynamic celestial events for the ValorSky Minecraft network, targeting **Folia 1.21.11**.
...
Version 2.2.0 resolves the actual solid ground before descent, waits for the display teleport to reach the surface, then plays the supplied impact stages. By default it creates a configurable Folia-safe crater with volcanic rock, an obsidian core and a loot chest. Terrain changes are skipped inside WorldGuard regions and can be disabled. The client pack remains the automatically distributed 2.1.6 pack.
