...
**Version 2.1.6** — Dynamic celestial events for the ValorSky Minecraft network, targeting **Folia 1.21.11**.
...
Version 2.1.6 uses a dedicated compact meteor during descent, then replaces it with the complete supplied 46-element impact scene at the announced coordinates. Build everything with `./gradlew build dist`; the client pack is written to `build/distributions/ValorSky-SkySignals-ResourcePack-2.1.6.zip`.
